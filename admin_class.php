...
Class Action {
	...
	function save_information(){
		...
		// echo $data;exit;
		if(empty($id)){
			$save = $this->db->query("INSERT INTO info_list set $data");
		}else{
			$save = $this->db->query("UPDATE info_list set $data where id = $id");
		}
		if($save){
			return 1;
		}
	...
	function delete_information(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM info_list where id = $id");
		if($delete){
			return 1;
		}
	}
